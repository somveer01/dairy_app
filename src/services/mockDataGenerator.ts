import { Customer, MilkEntry, MilkType } from '../types';
...
export const generate50Customers = (supplierId: string = 'supp_default_1'): Customer[] => {
  return INDIAN_NAMES.map((name, index) => {
    const custNum = index + 1;
    const milkType: MilkType = index % 2 === 0 ? 'cow' : 'buffalo';
...
export const generateDailyDeliveriesForCustomers = (
  ...
  supplierId: string = 'supp_default_1'
): MilkEntry[] => {
